...
//
//case class McodState(var PD: mutable.HashMap[Int, Data], var MC: mutable.HashMap[Int, MicroCluster], var PDTree: MTree[Data], var MCMtree: MTree[Data])
//
//case class MicroCluster(var center: ListBuffer[Double], var points: ListBuffer[Data])
//
//class Pmcod_mtree_PDMC(time_slide: Int, range: Double, k_c: Int) extends ProcessWindowFunction[(Int, Data), (Long, Int), Int, TimeWindow] {
//
...
//      val PDTree = new MTree[Data](k, DistanceFunctions.EUCLIDEAN, mySplit)
//      //===========END PD MTREE Test
//      //===========MC MTREE Test
//      val MCTree = new MTree[Data](k, DistanceFunctions.EUCLIDEAN, mySplit)
//      //===========END MC MTREE Test
//      val PD = mutable.HashMap[Int, Data]()
//      val MC = mutable.HashMap[Int, MicroCluster]()
//      val current = McodState(PD, MC, PDTree, MCTree)
...
//        //===========MC MTREE Test
//        val tmpData = new Data(state.value().MC(mc).center, 0L, 0, mc)
//        state.value().MCMtree.remove(tmpData)
//        //===========END MC MTREE Test
...
//
//      //===========PD MTREE Test
//      val query: MTree[Data]#Query = state.PDTree.getNearestByRange(el, 3* R / 2)
//      val iter = query.iterator()
//      while (iter.hasNext) {
//        val node = iter.next()
//        val thisDistance = node.distance
//        val p = node.data
...
//    //===========MC MTREE Test
//    state.value().MCMtree.add(new Data(newMC.center, 0L,0, mc_counter))
//    //===========END MC MTREE Test
//    mc_counter += 1
//  }
//
...
//    //===========MC MTREE Test
//    val query: MTree[Data]#Query = state.value().MCMtree.getNearestByRange(el, 3* R / 2)
//    val iter = query.iterator()
//    while (iter.hasNext) {
//      val node = iter.next()
//      res+=((node.data.id, node.distance))
//    }
//    //===========END MC MTREE Test
//    res
//  }
